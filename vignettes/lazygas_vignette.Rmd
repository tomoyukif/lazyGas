...
Since v0.5.0, lazyGas also provides a **variant viewer** (`getVariantViewerData()`, `plotVariantViewer()`) and an **interactive dashboard** (`makeInteractiveDashboard()`) that links candidate genes to per-gene variant plots in a single HTML file. Runnable demos are available via `runDashboardDemo()` and, for v0.6+ features, `runMvpDemo()` using bundled files in `inst/extdata/`.
...
If you upgraded from an older lazyGas version that stored results under
`lazygas/` in the GDS, migrate them once into the companion store:

```{r eval=FALSE}
lg <- importLazyGasResults(lg)
```

`importLazyGasResults()` copies legacy GDS nodes when the companion store is empty;
if both exist, the companion store is kept and you can run import explicitly when needed.
<br>
...
(`demo_pheno_multitrait.csv`), gene annotation, SnpEff effects, and an ortholog
table (`demo_orthologs.csv`). It runs `runLazyGas()`, prints GWAS QC and
cross-trait peak summaries, conditional association and credible-set results,
and ortholog matches, then writes an HTML report.
...
| `demo_orthologs.csv` | Ortholog IDs and scores for `annotateOrthologs()` |
...
Ortholog annotation on a candidate table:

```{r eval=FALSE}
ortholog <- read.csv(system.file("extdata", "demo_orthologs.csv", package = "lazyGas"))
cand <- lazyData(lg, dataset = "candidate", pheno = "fruit_weight")
annotateOrthologs(candidate = cand, ortholog = ortholog)
```

Shiny and Quarto templates ship under `inst/shiny/` and `inst/quarto/`.
Launch the Shiny explorer with `runLazyGasShiny()`.
