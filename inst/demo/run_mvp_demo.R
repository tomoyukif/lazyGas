...
#   Rscript inst/demo/run_mvp_demo.R [output_dir]
# Or from R after devtools::load_all():
#   runMvpDemo()

args <- commandArgs(trailingOnly = TRUE)
out_dir <- if (length(args) >= 1L) args[[1L]] else file.path(getwd(), "demo_output", "mvp")
...
if (!exists("runMvpDemo", mode = "function")) {
...
if (!exists("runMvpDemo", mode = "function")) {
  stop(
    "runMvpDemo() not found. Run from the package root with devtools::load_all(), ",
    "or install lazyGas with demo extdata files.",
    call. = FALSE
  )
}

runMvpDemo(out_dir = out_dir)
